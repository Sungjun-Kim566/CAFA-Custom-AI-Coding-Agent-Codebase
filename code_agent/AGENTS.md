...
1. `prompts/`: stores instructive markdown files that configure the agent's behavior on its response generation.
    - `prompts/coding-agent.md`: Drives the **design** and **code** stages of the workflow. Produce the Blueprint
    - `prompts/reviewer.md`: Drives the **review/revise** stage. Re-validates the final JSON, then package the response.
...
Keep initial context minimal. Use `wiki/index.md` to route to specific CAFA references only when needed. Use `projects/<agent-slug>/` as the source of truth for active debugging, revision, and generated-agent state. Do not load the full wiki unless the task explicitly requires it.
